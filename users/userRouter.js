...
router.get('/:id/posts', validateUserId, (req, res) => {
  ...
  Users.getUserPosts(id)
    ...
    .catch(error => {
      res.status(500).json({error: 'Error retrieving these messages.'});
    });
});
...
router.delete('/:id', validateUserId, (req, res) => {
  ...
  Users.remove(id)
    ...
    .catch(error => {
      console.log(error);
      res.status(500).json({error: 'Error deleting the requested record.'});
    });
});
...
router.put('/:id', validateUserId, validateUser, (req, res) => {
  ...
  Users.update(id, req.body)
    ...
    .catch(error => {
      console.log(error);
      res.status(500).json({error: 'Error updating the current user.'});
    });
});
